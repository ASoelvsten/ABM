...
#    q, bins, patches = plt.hist(q, bins =50, range=(0,0.5), density=True)
#    plt.close()
#    q = q/np.sum(q)
#    qtr = np.log10(q[abs(bins[:-1]+0.005-truth).argmin()])
    return -qtr[0]
...
for jot, obs_name in enumerate(data):
    ...
    if os.path.isfile("../DIST/summary_"+name+".txt"):
        ...
        for i in range(5):
            s_median = np.percentile(samples[:,i],50)
#            sstd1 = np.percentile(samples[:,i],16)
#            sstd2 = np.percentile(samples[:,i],84)
#            sstd = (sstd2-sstd1)/2
            obs.extend([s_median])
#            std.extend([sstd])
